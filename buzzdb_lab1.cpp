 ...
 *    - read only makes more efficient, const string& s
 *
 * 5. RAII (Resource Acquisition Is Initialization):
 *    - Objects clean up automatically when they go out of scope, dont have to malloc
 *    - No garbage collector like Java/Python - deterministic destruction
 *    - fstream files close automatically when the object is destroyed  - auto closes files when obj is destroyed
 *
 * 6. STL CONTAINERS:
 *    - std::vector<T>: Dynamic array (like ArrayList<T> in Java)     vector<int> is like ArrayList<Integer>
 *    - std::map<K,V>: Ordered map (like TreeMap<K,V> in Java)
 *    - std::unordered_map<K,V>: Hash map (like HashMap<K,V> in Java)
 *    - std::pair<A,B>: Tuple of 2 elements           a tuple in python
 *
 * 7. THREADING:
 *    - std::thread for creating threads (like Thread in Java)
 *    - std::mutex for locks (like synchronized in Java)               for locks?
...
// usually better to include std everywhere
using namespace std;
 ...
 *
 * strucs are PUBLIC java classes
 *
 ...
 *
 * cant do int x; like java, must do int x = 0;
 */
struct User
{
...
private:
    ...
    static string trim(const string &s)
    {
        ...
        // NOTE: The argument should be the CHARACTERS TO SKIP, not the string itself!
